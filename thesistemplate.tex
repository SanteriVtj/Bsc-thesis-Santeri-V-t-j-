...
\subsection*{Graphoids} \label{app:gmp} \addcontentsline{toc}{subsection}{Graphoids} 

\subsection*{Global Markov property} \label{app:gmp} \addcontentsline{toc}{subsection}{Global Markov property} 

Global Markov property is generalization of local Markov property where the independency is guaranteed for any subsets of variables within graph that are wholly separated from each other by third subset of variables.

\begin{itemize}
    \item[] For distribution $\mu(\bm{\cdot})$ subset of nodes $X$ and $Y$ separated by $Z$ in $\g$.
    \begin{gather*}
        \mu(v_{X}, v_{Y}|v_{Z})=\mu(v_{X}|v_{Z})\mu(v_{Y}|v_{Z})
    \end{gather*}
\end{itemize}
